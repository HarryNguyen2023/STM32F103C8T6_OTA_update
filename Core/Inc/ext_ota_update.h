...
#define EXT_CONFIG_FLASH_ADD    0x08004C00  // 6 kB
#define EXT_APP_START_ADD	    0x08006400  // 13 kB
#define EXT_APP_SLOT0_FLASH_ADD 0x08009800  // 13 kB
#define EXT_APP_SLOT1_FLASH_ADD 0X0800CC00  // 13 kB

// Size of memory layout
#define CONFIG_FLASH_SIZE	6
#define DATA_FLASH_SIZE		13

// Slot characteristic
#define EXT_SLOT_NO         2
#define EXT_SLOT_MAX_SIZE   (13 * 1024)
...
// Reboot reason
#define EXT_FIRST_TIME_BOOT   ( 0xFFFFFFFF )
#define EXT_NORMAL_BOOT       ( 0xBEEFFEED )
#define EXT_OTA_REQUEST       ( 0xDEADBEEF )
#define EXT_LOAD_PREV_APP     ( 0xFACEFADE )
...
typedef struct
{
  uint8_t is_this_slot_valid;
  uint8_t is_this_slot_active;
  uint8_t should_we_run_this_slot_fw;
  uint32_t fw_size;
  uint32_t fw_crc;
  uint32_t reserved_1;
  uint32_t reserved_2;
  uint32_t reserved_3;
}__attribute__((packed)) EXT_SLOT;

// General configuration
typedef struct
{
  uint32_t reboot_cause;
  EXT_SLOT slot_table[EXT_SLOT_NO];
}__attribute__((packed)) EXT_GNRL_CONFIG;
...
void EXT_OTA_Load_New_App(void);
